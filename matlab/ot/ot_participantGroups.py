...
# Contains:
#   rating_mats: stim x stim x groups (2) x groupings
source_file = 'preference_groups.mat'

data = loadmat(source_file);
rating_mats = data['rating_mats']

# rating_mats shape: stim x stim x participants
n_stim, _, n_groups, n_groupings = rating_mats.shape
...
epsilons = np.logspace(-2, 1, 20, base=10)
n_eps = len(epsilons)
...
OT = {
    'plan': np.full((n_stim, n_stim, n_groupings, n_eps), np.nan),
    'dist': np.full((n_groupings, n_eps), np.nan),
    'time': np.full((n_groupings, n_eps), np.nan)
    }

for pair in range(n_groupings):
    for e, epsilon in enumerate(epsilons):
        start_time = time.time()
        
        C1 = rating_mats[:, :, 0, pair]
        C2 = rating_mats[:, :, 1, pair]
        ...
        print(f"OT | pair={pair}/{n_groupings-1}, epsilon={epsilon} | duration={end_time - start_time:.3f}s")
    
##=== Save results to .mat
savemat('OT_preference_groups.mat', {
    'OT': OT,
    'epsilons': epsilons
})
